...
//this function returns the currenly assigned avatar for each player
function getAvatars() {
...
// this function will get the array of the current board and check the proposed move for a validity
...
function checkForWinCon() {
    ...
    info = info.split(','); //replace the string by commas into an array
    info.sort(); //sprt the square array in order despite the actual gameplay sequence
    for (var i in info) {
        squareArray.push(info[i].charAt(0)); //new array with only squares not avatars 
    }
    //call this function following array of functions to check for any of the possible win cons
...
//call this function to check board state for any ties and at accordingly
function check4Tie() {
    var boardState = document.getElementById("boardState").innerHTML;
    boardState = boardState.substring(1); //remove leading comma
    boardState = boardState.split(','); //separate the string by ommas into an array
    ...
    if (boardState.length >= 9 && check != "That's three in a row, Player 1 wins!" && check != "That's three in a row, Player 2 wins!") {
        ...
        tieSound(); //play a sound when a tie is detected
        writeMsg(txt1);
        setTimeout(function() {stopGame();}, 3000);
    }
}

// whenever a win is detected the corresponding function will call this 
//function to produce the following winning process for the game
...
function checkWinCon1(info,squareArray) {
	...
	// index 0, index 1 and index 2 and once they they do appear in the array, record their 
